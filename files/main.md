...
#### 最终结果：
- 验证： 研究人员首先用此方法测试了前一阶段的48所大学，结果“完美一致”，证明了该方法的可靠性 。

- 大规模测试：他们对Tranco Top 10,000域名中 6,917个有效域名进行了测试 。
...
> **核心问题：** 为什么在公共和私有邮件服务中，SMTP走私漏洞都如此普遍？研究人员发现，这并非成千上万个服务各自犯了错，而是因为电子邮件生态系统`高度集中`，问题出在了几个核心的基础设施上 。
...
1. **开源邮件软件 (Postfix, Sendmail, Exim等)：**
...
1. **邮件安全网关 (TrendMicro, Sophos, Proofpoint等)：**
...
> 论文针对SMTP走私漏洞，从“接收方”和“发送方”两个角度给出了明确的修复建议。

#### 1. 对接收方 (Receiving MTA) 的建议

-  **根本解决方案：严格遵守标准** 
   - 接收邮件服务器(MTA)应严格执行RFC 5321标准，**只承认 `\r\n.\r\n`** 作为唯一的DATA命令结束标记 。
   - *挑战*：困难在于更新那些为了兼容性而故意接受 `\n` 等非标符号的“遗留系统” 。RFC标准其实早已警告过这种做法会带来问题
    
- **短期解决方案：禁用流水线机制 (Disabling Pipeline)** 
  - 攻击者利用SMTP命令流水线机制来注入第二封（走私的）邮件 。
...
- **首选解决方案 (推荐)：数据编码 (Data Encoding)** 
  ...
  - **好处：** 编码后的数据（一长串无意义字符）**绝对不可能**被误认为是SMTP命令 。此方法完全符合RFC标准且兼容现有系统 。
- **短期修复：点填充 (Dot Stuffing) 和 字符串过滤** 
  - *点填充*：在行首的 `.` 前再加一个 `.`，这是RFC 5321规范的操作 。
  - *字符串过滤*：过滤掉可疑字符（如 `\r`, `\n`, `\x00`）。
  - *缺点*：这些**不是根本的修复方法** 。实验证明（第3.2节），它们可以被**空字符 (`\x00`)** 绕过，因为 `\x00` 会干扰服务器的行识别 。
